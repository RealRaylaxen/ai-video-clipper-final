...
def upload_to_free_cloud(file_path: str) -> str:
    """Upload completed clip to catbox.moe via curl, return resulting link"""
    ...
    # Upload to catbox.moe using POST with reqtype=fileupload
    cmd = [
        "curl", "-s",
    ...
    # catbox.moe returns the file URL directly on success
    if output.startswith("https://"):
        print(f"[OK] Upload complete!")
        return output

    # If not a URL, return the raw response for debugging
